...
export async function waitForUser(user: User, refresh: boolean, setRefresh: (refresh: boolean) => void, calledFrom: string = "") {
    await delay(100);
    let count = 0;
    if (user.email !== "") {
      console.log(`[waitForUser${calledFrom !== "" ? ` <- ${calledFrom}` : ""}] user is already ready`);
      return;
    }
    while (auth.currentUser === null) {
      if (count > 10) {
        console.log(`[waitForUser${calledFrom !== "" ? ` <- ${calledFrom}` : ""}] user is not logged in`);
        setRefresh(!refresh);
        return;
      }
      count++;
      console.log(`[waitForUser${calledFrom !== "" ? ` <- ${calledFrom}` : ""}] waiting for user`);
      await delay(100);
    }
    while (user.email === "") {
      if (count > 10) {
      ...
      count++;
      console.log(`[waitForUser${calledFrom !== "" ? ` <- ${calledFrom}` : ""}] waiting for user data`);
      await delay(100);
    }
    console.log(`[waitForUser${calledFrom !== "" ? ` <- ${calledFrom}` : ""}] user is ready`);
    setRefresh(!refresh);
  }

async function getDBUser() {
    if (auth.currentUser === null) {
        console.log("[user] getDBUser has been called and user is not signed in. This should be impossible.");
        return null;
    }
...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        ...
        if (user.email !== localUser.email) {
            ...
            console.log("[user] AuthStateChanged and user is already in UserSingleton");
        }
    } else {
...
export class UserSingleton implements User {
    ...
    public static addObserver(observer: () => void) {
        let localUser = UserSingleton.getInstance();
        localUser.observers.push(observer);
    }

    public static removeObserver(observer: () => void) {
        let localUser = UserSingleton.getInstance();
        localUser.observers = localUser.observers.filter((o) => o !== observer);
    }

    public static clearObservers() {
        let localUser = UserSingleton.getInstance();
        localUser.observers = [];
    }

    public static notifyObservers() {
        let localUser = UserSingleton.getInstance();
        localUser.observers.forEach((o) => o());
    }
